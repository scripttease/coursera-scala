 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("list with repeated elements") {
    assert(sum(List(2,3,0,0,4,3)) === 12)
  }
  ...
  test("max with zeros") {
    assert(max(List(2,0,4,-1,3,9,0,2,8,2,2)) === 9)
  }

  test("max with negative numbers") {
    assert(max(List(-3,-7,7,19)) === 19)
  }

  test("max all negative numbers") {
    assert(max(List(-7,-7,-5,-19)) === -5)
  }

  test("max repeated negative numbers") {
    assert(max(List(-3,-19,-7,-7,-1)) === -1)
  }
